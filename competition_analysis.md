...
## 既存LoRAをomniセットで測定: 弱い2次元に一切効いていない（2026-08-06）

`libero_omni`（31件）でベースとLoRA（40,000ステップ、マージ済み）を同条件で比較した。

| カテゴリ | タスク数 | ベース | PARC LoRA | Δ |
|---|---|---|---|---|
| Background Textures | 7 | 85.7% | 71.4% | −14.3 |
| Light Conditions | 7 | 76.2% | 85.7% | +9.5 |
| Objects Layout | 8 | 29.2% | 29.2% | ±0 |
| Language Instructions | 9 | 11.1% | 11.1% | ±0 |
| **Overall** | **31** | **47.3%** | **46.2%** | **−1.1** |

- **Language Instructionsは9タスク全てで成功率が完全に一致**した。LoRAがこの次元に
  一切影響していない。
- Objects Layoutはカテゴリ平均こそ同じだが、個別タスクは±33〜67ppと激しく動いており
  （4件が変化）、実体は改善ではなくノイズで相殺されているだけ。
- 31タスク中22タスクが完全同一の成功率。動いた9タスクも±33.3pp（3エピソード刻みで
  1本ぶん）が中心で、方向も揃っていない。

### 効かなかった理由（データセットを直接確認して判明）

学習に使った`lerobot/libero_plus`のローカルキャッシュ
（`~/parc_lora_workspace/lerobot_cache/`）を開いて数えた結果:

1. **データセットに言い換え指示が1件も存在しない**。`meta/tasks.parquet`の
   ユニークなtask labelは**40件だけ**で、すべて素の指示文。うち30件が
   `smolvla_parc_lora_config.yaml`の`suite_task_names`と正規化一致する。
   つまり**このデータセットで学習する限り、言語ロバスト性は原理的に獲得できない**。
   一方、評価側のLanguage変種（`_language_N.bddl`）は言い換えられた指示文を持つ。
2. **学習に使ったエピソードが極端に少ない**。データセットは
   `total_episodes: 14347` / `total_frames: 2,238,036`（40タスク、1タスクあたり
   約360エピソード＝これが背景・照明・物体配置の摂動バリエーション）あるのに、
   config は `train_episodes_per_task: 5`。**30タスク × 5 = 150エピソード**しか
   使っていない（全体の約1%）。摂動の多様性をほぼ見せずに40,000ステップ回していた。

### 含意

- 「LoRAが効かなかった」のではなく、**効きようのない条件で学習していた**。
  `train_episodes_per_task`を大幅に増やす（データはローカルに揃っている）だけで
  Objects Layout / Background Textures は伸びる余地がある。
- **言語ロバスト性はこのデータセットでは解決できない**。評価側の
  `_language_N.bddl`から言い換え文を取り出して学習データのtask labelに
  差し込む（同一エピソードに複数の言い換えを割り当てる）等、別の手当てが要る。
- カメラ視点・ロボット初期状態は、評価すらローカルでできていない（実体ファイルも
  ハーネスの実装も無い）。本番で最も落としている可能性が高いのはここだが、
  現状は打ち手の効果を測る手段が無い。
