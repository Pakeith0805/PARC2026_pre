...
## IS_SERVER_HANDOFF.md

IS計算機サーバー側で作業する人／AIエージェント向けの申し送り。
2026-08-06時点の依頼内容は「データセットの残り約8GBを落として、
`train_episodes_per_task` を 5 → 200 に増やして再学習する」。

背景（既存LoRAが効いていないこと、その原因が学習エピソード数の
不足＝全体の約1%しか使っていなかったこと）と、`train_episodes_per_task`
の上限が200である理由（Notebookセル12の件数検証で落ちるため）も
そこに書いてある。
